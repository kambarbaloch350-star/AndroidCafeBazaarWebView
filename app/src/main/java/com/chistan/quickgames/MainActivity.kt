...
class MainActivity : AppCompatActivity(), WebAppBridge.HostListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 1b. Offline notifications – track open time and ensure scheduled
        try {
            GameNotificationManager.updateLastOpenTime(this)
            GameNotificationManager.createChannels(this)
        } catch (e: Exception) {
            Log.w(TAG, "Notification setup failed: ${e.message}")
        }
    ...
    override fun onResume() {
        ...
        // Track for offline re-engagement notifications
        try {
            GameNotificationManager.updateLastOpenTime(this)
        } catch (e: Exception) {
            Log.w(TAG, "Failed to update last open: ${e.message}")
        }
